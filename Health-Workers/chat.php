...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['message'])) {
    ...
    if ($message) {
        mysqli_query($conn, "INSERT INTO chat_messages (sender_id, receiver_id, message) VALUES ({$_SESSION['user_id']}, $receiver_id, '$message')");
    }
...
if ($active_chat) {
    mysqli_query($conn, "UPDATE chat_messages SET is_read=1 WHERE sender_id=$active_chat AND receiver_id={$_SESSION['user_id']}");
}

$users = mysqli_query($conn, "SELECT u.*, (SELECT COUNT(*) FROM chat_messages WHERE sender_id=u.id AND receiver_id={$_SESSION['user_id']} AND is_read=0) as unread FROM users u WHERE u.id != {$_SESSION['user_id']} AND u.status='active' ORDER BY u.first_name");

$messages = [];
if ($active_chat) {
    $msgs = mysqli_query($conn, "SELECT cm.*, CONCAT(u.first_name,' ',u.last_name) as sender_name FROM chat_messages cm JOIN users u ON cm.sender_id=u.id WHERE (cm.sender_id={$_SESSION['user_id']} AND cm.receiver_id=$active_chat) OR (cm.sender_id=$active_chat AND cm.receiver_id={$_SESSION['user_id']}) ORDER BY cm.created_at ASC");
...
<main class="ml-64 p-6">
    ...
    <div class="bg-white rounded-xl shadow-sm overflow-hidden flex" style="height:calc(100vh - 160px)">
        <div class="w-72 border-r overflow-y-auto">
            <div class="p-4 border-b"><h3 class="font-semibold text-sm text-gray-700">Contacts</h3></div>
            ...
            <a href="?user=<?=$u['id']?>" class="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 border-b <?=$active_chat==$u['id']?'bg-teal-50 border-l-4 border-l-teal':''?>">
                ...
                <div class="flex-1 min-w-0"><p class="text-sm font-medium truncate"><?=htmlspecialchars($u['first_name'].' '.$u['last_name'])?></p><p class="text-xs text-gray-400"><?=ucfirst($u['role'])?></p></div>
        ...
        <div class="flex-1 flex flex-col">
            ...
            <div class="p-4 border-b flex items-center gap-3">
                ...
                <div><p class="font-semibold text-sm"><?=htmlspecialchars($chat_user['first_name'].' '.$chat_user['last_name'])?></p><p class="text-xs text-gray-400"><?=ucfirst($chat_user['role'])?></p></div>
            </div>
            <div class="flex-1 overflow-y-auto p-4 space-y-3" id="chatBox">
                <?php foreach ($messages as $m): $mine = $m['sender_id'] == $_SESSION['user_id']; ?>
            ...
            <?php else: ?>
            <div class="flex-1 flex items-center justify-center text-gray-400"><div class="text-center"><i class="fas fa-comments text-5xl mb-3"></i><p>Select a contact to start messaging</p></div></div>
            <?php endif; ?>
        </div>
    </div>
</main>
